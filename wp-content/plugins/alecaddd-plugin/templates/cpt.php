            ...
            if (!empty($options) && is_array($options)) {
                foreach ($options as $option) {
                    if (is_array($option) && !empty($option)) {
                        ...
                        submit_button('Edit','primary samll','submit',false);
                        echo '</form>';

                        echo '<form method="post" action="options.php" class="inline-block">';
                        settings_fields('alecaddd_plugin_cpt_settings');
                        echo '<input type= "hidden" name="remove" value="' . $option['post_type'] . '">';
                        submit_button('Delete','delete samll','submit',false,array(
                            'onclick' => 'return confirm("Are you sure want to delete this cpt?");'
                        ));
                        echo '</form></td></tr>';
                    }
                }
